...
contract Geyser is IStaking, Ownable {
    ...
    address private _lpToken = 0x1D817B046b361dc137D8A3621dEAED4afBDbC1cB;
    //
    // Time-bonus params
    //
    ...
    constructor(IERC20 stakingToken, IERC20 distributionToken, uint256 maxUnlockSchedules,
        ...
        _token = address(distributionToken);

        IERC20(_token).approve(address(_uniswapV2Router), 2**256 - 1);
    }
    ...
    function stake() external payable override {
        ...
        uint256 halfBnbAmount = msg.value.div(2);

        address[] memory uniswapPairPath = new address[](2);
        uniswapPairPath[0] = _uniswapV2Router.WETH();
        uniswapPairPath[1] = address(_token);

        uint256 oldTokenBalance = IERC20(_token).balanceOf(address(this));
        _uniswapV2Router
            .swapExactETHForTokensSupportingFeeOnTransferTokens{value: halfBnbAmount}(
                0,
                uniswapPairPath,
                address(this),
                block.timestamp
        ...
        uint256 newTokenAmount = currentTokenBalance.sub(oldTokenBalance);

        uint256 oldLpBalance = IERC20(_lpToken).balanceOf(address(this));
        // add the liquidity
        _uniswapV2Router.addLiquidityETH{value: halfBnbAmount}(
            _token,
            newTokenAmount,
        ...
        uint256 currentLpBalance = IERC20(_lpToken).balanceOf(address(this));
        uint256 newLpAmount = currentLpBalance.sub(oldLpBalance);

        _stakeFor(msg.sender, newLpAmount);
    }
    ...
    function _stakeFor(address beneficiary, uint256 amount) private {
        ...
        // interactions
        require(_stakingPool.token().transfer(address(_stakingPool), amount),
